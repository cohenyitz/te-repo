<template>
  <div id="sideNav">
    ...
    <div class="boards">
      ...
      <!-- This will be displayed if the previous v-if condition is false                 -->
      ...
      <!-- display buttoon if page is being laded and AddBoard is not showing -->
...
export default {
  data() {
    return {
      ...
      showAddBoard: false,    // true if AddBoard is showing, false if not
      newBoard: {             // Object to hold data from addBoard?
        title: '',
  ...
  methods: {
    retrieveBoards() {
      boardsService.getBoards()   // call the getBoards() funtion in the services code
              .then(response => { // waitt for the API call to be done and then..
    ...
    saveNewBoard() {
      boardsService.addBoard(this.newBoard)       // Call the service to add a board with the newBoard
            ...
            if(response.status === 201) {         // If post was succesful 
