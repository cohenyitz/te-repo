 ...
 */
function takeOptionalParameter(sample = 0) {
  return sample;
}
/**
 ...
 */
/**
 * function converts temperature  between both F and C. Needs to be passed in a
 * temperature {number} and temperatureUnit{string}. returns converted temperature and
 * temperatureUnit (if set to true).
 * 
 * @param {number} temperature takes in the temperature to be converted 
 * @param {string} temperatureUnit takes the unit (ie F || C) of temperature to be converted
 * @param {string} [includeUnit = false] includeUnit default value of unit to be included in return is false
 * @return {number, string} returns new converted temperature and the new temperature unit if set to true 
 */
