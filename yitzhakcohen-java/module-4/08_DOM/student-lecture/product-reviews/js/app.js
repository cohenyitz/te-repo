...
function setPageTitle() {
  ...
  const pageTitle = document.getElementById('page-title');  // get a reference to id='page-title in the DOM
  // pageTitle now points to the element in the DOM with the id='page-title'

  pageTitle.querySelector('.name').innerText =name; // go to the element referenced by pageTitle
...
function setPageDescription() {
  document.querySelector('.description').innerText = description; // Get a reference to the element with class='description'
                                                                  // Set its innerText to our description variable
}
...
function displayReviews() {
  ...
  reviews.forEach((aReview) => {  // Loop through the array called views with each element named aReview in the loop
      ...
      addReviewer(newReview,aReview.reviewer);  // Call the addReviewer() with the parent and the data
      addRating(newReview, aReview.rating);     // Call the addRating() with the parent and the rating
      addTitle(newReview, aReview.title);       // Call addTitle() with the parent and the title 
      addReview(newReview, aReview.review);     // Call addReview () with the parent and the review
...
function addRating(parent, numberOfStars) {
  ...
  rating.setAttribute(`class`, `rating`);         // Add a class='rating' to the new div
  for(let i = 0; i <numberOfStars; i++) {         // Loop to put one star image for rating value
     const star = document.createElement('img');  //    create an img element for a star
     star.src='img/star.png';                     //    set the src= for the img to the star picture
...
function addTitle(parent, title) {
  ...
  parent.appendChild(h3);                      // New h3 to parent
}
