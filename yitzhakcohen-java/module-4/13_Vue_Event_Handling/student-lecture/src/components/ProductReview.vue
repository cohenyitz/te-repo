  ...
  <div class="main">
    ...
    <div class="well-display">
      ...
      <div class="well">
        ...
        <!-- add an 's' at the end of words if the is more than one star -->
        <!-- the conditional operater -? example                         -->
        <!-- format -- condition ? result-if-true : result-if-false      -->
        <!-- used here to provide condiontal output                      -->
    ...
    <!--      when the v-if condition is true - we include the element in the DOM   -->
    <!--      when the v-if condition is false - we remove the element in the DOM   -->
    ...
    <!-- if the form is showing, thos link isn't                                    -->
    <a
      id="show-form-button"
      href="#"
      ...
      v-if="!showForm"
    >Show Form</a>
    <!-- use a v-if with showForm variable to show/hide this form                   -->
    <!-- if showForm variable is true, the form displays, if false, it doesn't      -->
    <!-- use the v-on to add a click event handler when the submit event occurs     -->
    ...
    <form v-if="showForm === true" v-on:submit.prevent="addNewReview">
      ...
      <div class="form-element">
        ...
        <!-- .number on v-model modifier says only accept numeric values in the field -->
      ...
      <!-- Define a canel button with a click event handler to call resetForm()       -->
...
export default {
  ...
  data() {
    return {
      ...
      filter : 0,         // control the ratings displayed  -- what rating do they want to see
  ...
  // computed values are recreated everytime something changes on the page
  computed: {
    averageRating() {
  ...
  // methods run only when called - do not forget the 's' on methods
  methods: {
    filteredReviews() {   // return an array of reviews with only the ratings requested
      ...
      /* Long-form technique
         ...
         if (this.filter === 0) {   // if filter is 0, include the element
           return true;
         }
    ...
    numberOfReviews(reviews, starType) {    // recieving the array and the rating 
      return reviews.reduce((currentCount, review) => {
        // add a 1 or a 0 to current count depending on if the review.rating matches the starType we passed in. 
        return currentCount + (review.rating === starType ? 1 : 0);
        // alternate technique for adding to current count
        // uses the fact that in JavaScript true=1 and false=0 (truthy/falsey concepts)
        //      if rating === starType -it equals to true(ie 1) if rating != starType - it equals to false(ie 0)
    ...
    addNewReview() {
      ...
      this.reviews.unshift(this.newReview); // add new review to beggining of array (unshift)
