<template>
  <div id="sideNav">
    ...
    <div class="hotels">
      ...
      <div>
        <!-- add a router link for each hotel in the hotels array of Vuex Data Store -->
        <!-- list the name as the link-->
        <router-link
...
export default {
  ...
  created() {
    // This is a Vue lifecycle hook - do this before the page is displayed
    this.retrieveBoards(); //    run the retireveBoards method
    this.isLoading = false;
  },
  methods: {
    retrieveBoards() {
      // run from the created hook
      HotelService.getHotels() // Run/ call the HoteService  getHotels() functions
        .then(response => {
          // wait for Promise from the API call to complete
          this.$store.commit("SET_HOTELS", response.data); //call the SET_HOTELS  mutation
