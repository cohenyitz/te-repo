...
--
-- The database manager does not guarantee the order of the rows in the result unless you have an ORDER BY
--
-- The same query with out an ORDER BY may return the same order of results 100 times in a row
--      the 101st time could be different
--
-- If the order of rows in the result matter - code an ORDER BY - DO NOT ASSUME THE ORDER IS ALWAYS THE SAME
--
-- null values are sorted as group at the beginning of result - if included
...
-- Show Populations of all countries in acscending order only if population is greater than 1000000
select name, population
  from country
...
select name
      ,population / surfacearea  -- derived column does not have a name
  from country
 order by 2          -- order by column number
;  
select name
      ,population / surfacearea  as pop_density -- derived column can be assigned a name
 --   ,population    
 --   ,surfacearea
  from country
 order by pop_density -- order by using derived name
;
--------------------------------------------------------------------------------------------------------
...
--
-- LIMIT is a postgreSQL word - all database managers have a way to LIMIT results - might called something else
--
...
--
select distinct name, lifeexpectancy  -- distinct is applied to the combination of all columns in the select
  from country
...
--
select distinct continent  -- distinct is applied to the combination of all columns in the select
  from country
;
...
--
-- null values are ignored by all aggregate functions
--  
--
-- Show average life expectancy in the world
select avg(lifeexpectancy)  -- avg of non-null lifeexpecancy of the entire table
  from country
;                           -- no group by specified so on value for entire table is generated

-- Show average life expectancy of each continent
select continent, avg(lifeexpectancy)  -- avg of non-null lifeexpecancy 
  from country
...
--         named in the group by or in in the functio.
--
-- column in the SELECT tells SQL you want one row in the result for every row that matches the where clause
-- GROUP BY tells SQL you want one row in the result for each disctinct value for column in the function
--
-- conflicting requests when column not in group by is coded with aggregate function
--
--
...
select sum(population) as Ohio_Population
      ...
      ,count(*)        as Num_Ohio_Cities
      ,min(population) as Smallest_Ohio_Pop
      ,max(population) as Largest_Ohio_Pop
      ,min(population) + max(population) as Billy_Pop 
  from city
where district = 'Ohio'
;
