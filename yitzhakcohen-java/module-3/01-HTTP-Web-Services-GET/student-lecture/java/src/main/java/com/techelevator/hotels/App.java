...
public class App {
    ...
    private static void run() {
        ...
        int hotelWeWant = 1;	// Somehow you get a value for the hotel id you want (from database, user keyboard, calculation etc..)
        int starsWeWant = 3;	// Somehow you get a value for the hotel id you want (from database, user keyboard, calculation etc..)
        ...
        while(menuSelection != 0) {
            ...
            System.out.println("");
            if (menuSelection == 1) {
            	// Call the hotel API to get all the hotels it knows about and pass it to prontHotels()
            	// Constructing the URL to be: "http://localhost:3000/hotels"
            	// 							   We tell RestTemplate we want an array of Hotel objects returned
            	// 							   You want it to turn JSON from the API into an array of Hotel objects
                printHotels(restTemplate.getForObject(API_BASE_URL + "hotels", Hotel[].class));			// Array of Hotel object returned
