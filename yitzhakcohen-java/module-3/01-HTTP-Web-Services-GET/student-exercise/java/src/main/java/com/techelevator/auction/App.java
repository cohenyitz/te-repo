...
public class App {
    ...
    public static Auction listDetailsForAuction() {	
    	System.out.println("Please enter number to list details for Auction: ");
    	int userInputParsed = 0;
        
        	try {
        		userInputParsed = Integer.parseInt(scanner.nextLine());
        	} 
        	catch(Exception e) {
        	System.out.println("Oh no! There was an error with your input. Please try again.");
        	}
        	
    	return restTemplate.getForObject(API_URL + "/" + userInputParsed, Auction.class);
    }

    public static Auction[] findAuctionsSearchTitle() {
        System.out.println("Enter search term to find Auction by title: ");
        
        String userInput = scanner.nextLine();
        Auction [] auction = null;
        try {
        	auction = restTemplate.getForObject(API_URL + "?title_like=" + userInput, Auction[].class);
        	if(auction.length == 0) {
        		System.out.println("No such entery found. Please input new search.");
        	}
        	} catch(Exception e) {
        	System.out.println("Oh no! There was an error with your input. Please try again.");
        }
    ...
    public static Auction[] findAuctionsSearchPrice() {
    	System.out.println("Find auctions based on price. Enter your desired price.");
    	double price = 0.0;
