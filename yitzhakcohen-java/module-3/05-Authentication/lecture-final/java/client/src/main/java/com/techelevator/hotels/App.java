    ...
    public static void main(String[] args) {
        ...
        while (menuSelection != 0) {
            ...
            try {
                ...
                } else if (menuSelection == 6) {                           // Login a user option
                    String credentials = consoleService.promptForLogin();  // Return user input as a String
                    if( credentials.split(",").length == 2 ) {             // split user input at , (username, password
                        ResponseEntity<Map> response = authenticationService.login(credentials);
                        if (response.hasBody()) {
                            String token = (String) response.getBody().get("token");  // Get the JWT from the response
                            hotelService.AUTH_TOKEN = token;                          // Set the HotelService AUTH_TOKEN to JWT from login
