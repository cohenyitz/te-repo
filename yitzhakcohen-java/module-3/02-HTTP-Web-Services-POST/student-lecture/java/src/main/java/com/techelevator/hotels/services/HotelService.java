...
public class HotelService {
  ...
  public Reservation addReservation(String newReservation) {
    ...
    // It's always good to be sure you have an object before you start processing 
    if (aReservation == null) {
    	System.out.println("Invalid reservation information enterd, please retry");	// We may add this to consoleService 
    	return null;
    }
    
    // Now that we have a reservation object, we need to send it to the API to add it to the Reservation resource
    ...
    theHeaders.setContentType(MediaType.APPLICATION_JSON); // Include in the headers that we are sending JSON formatted data
    													   // MediaTyoe is a group of valid constants for request data types
    
    // An HttpEntity is an object with properly formatted components of a request
    //	We use the HttpEntity so we don't have to know how to properly format the components of the request
    HttpEntity anEntity = new HttpEntity(aReservation, theHeaders);	//Construct an entity object for the request containing 
    																//	the data in the Reservation object and theheaders object we created
    
    // Call the API to add the reservation = it will return the reservation it added with the new Reservation Id
    // Use the postFor Object method in RestTemplate class - serialize and object and put data in request
    // 										   API-URL 					  headers+data, class of Object to be serialized 
    try {
  ...
  public Reservation updateReservation(String CSV) {
	  Reservation aReservation = makeReservation(CSV);	// Call a helper method (below) to make a reservation object from a string
	    ...
	    //	We use the HttpEntity so we don't have to know how to properly format the components of the request
	    HttpEntity anEntity = new HttpEntity(aReservation, theHeaders);	//Construct an entity object for the request containing 
	    																//	the data in the Reservation object and the headers we created
	    
	    // Call the API to add the reservation = it will return the reservation it added with the new Reservation Id
	    // Use the postFor Object method in RestTemplate class - serialize and object and put data in request
	    // 										   API-URL with id to update, 	 headers+data, class of Object to be serialized 
	    try {
	    	restTemplate.put(BASE_URL + "reservations/" + aReservation.getId() , anEntity, Reservation.class);	// Nothing is returned from a put 
  ...
  public void deleteReservation(int id) {
	  ...
	  try {
	  restTemplate.delete(BASE_URL + "reservations/" + id); // Ca;; the API with a HTTP DELETE request for the id given
	  }
	  catch (RestClientResponseException exceptionObject) {	// if there is a response error display status code and message
	    	console.printError(exceptionObject.getRawStatusCode() + exceptionObject.getStatusText());
	  } 
