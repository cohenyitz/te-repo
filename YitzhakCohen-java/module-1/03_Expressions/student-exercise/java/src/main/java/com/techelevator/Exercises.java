...
public class Exercises {
	...
	public int max1020(int a, int b) {
		...
		// if both are in range - return the larger
		if (aIsInRange && bIsInRange) {
			...
			// if only one is in range - return the one that is in range
			if (aIsInRange && !bIsInRange) {
				return a;
			}
			if (!aIsInRange && bIsInRange) {
			...
			// if neither is in range -return 0
			if (!aIsInRange && !bIsInRange) {
				return 0;
			}
	...
	public boolean cigarParty(int cigars, boolean isWeekend) {
		...
		boolean cigarsAboveRange = (cigars >= 40);		
		if ((!cigarsInRange) && (!isWeekend)) {
			return false;
		}
		if ((cigarsInRange) && (!isWeekend)) {
			return true;
		}
		if ((cigarsAboveRange) && (isWeekend)) {
			return true;
		}
	...
	public boolean squirrelPlay(int temp, boolean isSummer) {
		...
		boolean summerTempRange = (temp >= 60 && temp <= 100);
		if ((tempRange) && (!isSummer)) {
			return true;
		}
		if ((summerTempRange) && (!isSummer)) {
			return false;
		}
		if ((summerTempRange) && (isSummer)) {
			return true;
	...
	public int caughtSpeeding(int speed, boolean isBirthday) {
		...
		boolean bigTicket = (speed > 81);
	
		
		if ((smallTicket) && (isBirthday)) {
			return 0;
		}
		if ((smallTicket) && (!isBirthday)) {
			return 1;
		}
		if ((noTicket) && (!isBirthday)) {
			return 0;
		}
		return 0;
	}
	...
	public boolean in1To10(int n, boolean outsideMode) {
		...
		boolean nOutOfRange = ((n <= 1) && (n >= 10));
		if ((nInRange) && (!outsideMode)) {
			return true;
		} else if ((nInRange) && (outsideMode)) {
			return false;
		} else if ((nOutOfRange) && (!outsideMode)) {
			return false;
		} else if ((nOutOfRange) && (outsideMode)) {
			return true;
		}
	...
	public boolean specialEleven(int n) {
		boolean specialInt = ((n % 11 == 0) || (n % 11 == 0 + 1));
		if (specialInt) {
			return true;
		}
		return false;
	...
	public int teenSum(int a, int b) {
		boolean aLuckyTeen = ((a >= 13) && (a <= 19));
		boolean bLuckyTeen = ((b >= 13) && (b <= 19));
		int sum = a + b;
		if ((aLuckyTeen) || (bLuckyTeen)) {
			return 19;
		} else if (!(aLuckyTeen) && !(bLuckyTeen)) {
			return sum;
		}
		return 0;
	}
	...
	public int teaParty(int tea, int candy) {
		boolean partyBad = ((tea < 5) || (candy < 5));
		boolean partyGood = ((tea >= 5) && (candy >= 5));
		boolean partyGreat = ((2 *tea) > candy) || (2 * candy) > (tea);
		if (partyBad) {
			return 0;
		}
		if (partyGood) {
