...
public class JDBCCityDAO implements CityDAO {  // concrete class is named JDBCtablenameDAO
	...
	private JdbcTemplate myJdbcTemplate;  // declare a JdbcTemplate Spring DAO object
	
	// constructor that takes data source and assigns to JdbcTemplate object
	// User is expected to define a data source and pass it to the constructor
	...
	public void save(City newCity) {  // add a new city from a City object
		String sqlInsertCity = "INSERT INTO city(id, name, countrycode, district, population) " +
							   "VALUES(?, ?, ?, ?, ?)";
		// WE will get the next value for the CityID from the database and store it in the object the user pass us
		newCity.setId(getNextCityId());  //Go get the next serial value for the City table CityID
										 ...
										 //		So when the user calls us to save   what CityID 
	...
	public List<City> findCityByCountryCode(String countryCode) {
		ArrayList<City> cities = new ArrayList<>();  // Returned from method - ArrayList in case there is more than one
		
		String sqlFindCityByCountryCode = "SELECT id, name, countrycode, district, population "+
										   "FROM city "+
										   "WHERE countrycode = ?";
		
		// When a queryforRowSet is done, the result of the SELECTis returned in an SqlRowSet object
		...
		while(results.next()) {		// Loop while there is a row in the result and it retrieves the current row
	...
	private long getNextCityId() {	// get the next value from a sequence object
		...
		if(nextIdResult.next()) {	// No loop needed to get result from SELECT - only on row wil come back
