...
public class HotelService {
  ...
  //  public so application can set it when they get the JWT
  public static String AUTH_TOKEN = "";  // Hold the JWT from login so we can send it 
                                         // when do any interaction for hotels or reservations
  ...
  public HotelService(String url) {  // The server URL will be passed in as a parameter
    BASE_URL = url;
  }
  ...
  private HttpEntity<Reservation> makeReservationEntity(Reservation reservation) {
    ...
    headers.setBearerAuth(AUTH_TOKEN);    // Must add the Bearer Token header with the JWT 
