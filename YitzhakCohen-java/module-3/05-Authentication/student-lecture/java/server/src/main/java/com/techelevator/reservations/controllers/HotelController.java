...
@RestController
@PreAuthorize("isAuthenticated()")	// Asking Spring to check to see if whomever wants to use this method is authorized
...
public class HotelController {
     ...
     */
    /*
     *  Only allow users who are also Administrators the ability to DELETE a reservation
     *  
     *  An Administrator is a user with the role "ADMIN"
     */
    @PreAuthorize("hasRole('ADMIN')")		// Check to be sure authenticated user has the role 'ADMIN'
    @ResponseStatus(HttpStatus.NO_CONTENT)
    @RequestMapping(path = "/reservations/{id}", method = RequestMethod.DELETE)
    //
    // Spring Security has a special class called Principal that in effect represents the Current User
    //
    // You don't have to do anything except declare it as a parameter to a method
    //
    // .getName() returns the name of thePrincipal (ie the currently logged-in user)
    //
